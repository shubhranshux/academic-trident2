    ...
    <section className="relative w-full min-h-screen flex flex-col justify-center bg-white overflow-hidden pt-32 md:pt-40 pb-20 group">
      ...
      <div className="max-w-[1400px] mx-auto px-6 xl:px-12 w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
        ...
        <div className="text-left">
          ...
          <div className={`relative min-h-[140px] md:min-h-[180px] lg:min-h-[220px] mb-8 transition-all duration-1000 delay-150 ease-out transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
            {slides.map((slide, idx) => (
              <h1 
                key={idx} 
                className={`absolute top-0 left-0 w-full font-serif text-5xl md:text-7xl lg:text-[88px] font-black text-[#212529] leading-[1.05] tracking-tight transition-all duration-1000 ease-[cubic-bezier(0.25,1,0.5,1)] ${currentSlide === idx ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0 pointer-events-none'}`}
              >
                {slide.title1} <br />
        ...
        <div className={`relative transition-all duration-1000 delay-700 ease-out transform ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-12 opacity-0'}`}>
           ...
           <div className="relative aspect-[4/5] md:aspect-square overflow-hidden rounded-2xl shadow-2xl bg-[#EFE7DF]/20 group/slider">
              {slides.map((slide, idx) => (
                <img 
                  key={idx}
                  src={slide.image} 
              ...
              <div className="absolute bottom-8 right-8 bg-[#283B91] text-white p-6 rounded-xl hidden md:block z-20">
              ...
              <div className="absolute top-6 left-6 bg-white/90 backdrop-blur-sm text-[#212529] px-4 py-2 rounded-lg hidden md:block shadow-md z-20">
                <div className="text-[10px] font-black uppercase tracking-widest text-[#8B6E66]">Est. 1998</div>
              </div>
           </div>
        </div>
