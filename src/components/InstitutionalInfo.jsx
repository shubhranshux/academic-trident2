...
export default function InstitutionalInfo() {
  return (
    <section className="py-24 bg-[#EDF2F7] relative overflow-hidden">
        ...
        {infoData.map((item, idx) => (
          <div key={idx} className={`flex flex-col ${item.reverse ? 'lg:flex-row-reverse' : 'lg:flex-row'} w-full min-h-[500px] group`}>
            ...
            <div className="w-full lg:w-1/2 relative overflow-hidden h-[400px] lg:h-auto">
              ...
              {/* Geometric Angle Cut */}
              {item.reverse ? (
                <div className="absolute top-0 right-0 w-16 h-full bg-[#EDF2F7] transform skew-x-12 translate-x-8 z-10 hidden lg:block"></div>
              ) : (
                <div className="absolute top-0 left-0 w-16 h-full bg-[#EDF2F7] transform -skew-x-12 -translate-x-8 z-10 hidden lg:block"></div>
              )}
            ...
            {/* Content Half */}
            <div className={`w-full lg:w-1/2 flex flex-col justify-center p-12 lg:p-24 bg-[#EDF2F7] relative`}>
